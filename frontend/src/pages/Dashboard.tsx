  ...
  return (
    <div className="p-8 overflow-y-auto h-full pb-20">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-brand-400 to-purple-400">
            Ringkasan Penjualan
          </h1>
          <p className="text-text-muted mt-1">Pantau performa bisnis Anda secara real-time</p>
        </div>
        
        <div className="flex items-center gap-2 bg-surface-dark border border-white/10 rounded-xl p-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="glass-card p-6 border border-brand-500/30 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              <p className="text-brand-300 text-sm font-medium mb-1">Laba Bersih (Net Profit)</p>
          ...
          <div className="space-y-3 border-t border-white/10 pt-4">
            {data.paymentBreakdown?.map((item: any, index: number) => (
              <div key={index} className="flex justify-between items-center text-sm">
                ...
                <span className="text-brand-300 font-bold">Rp {item.amount.toLocaleString('id-ID')}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {data.bestSellers.map((item: any, index: number) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-brand-600/20 text-brand-400 flex items-center justify-center font-bold">
                    #{index + 1}
                  </div>
                  <div>
                ...
                <div className="text-right">
                  <p className="font-bold text-sm text-brand-300">Rp {item.revenue.toLocaleString('id-ID')}</p>
                </div>
              </div>
            ))}
