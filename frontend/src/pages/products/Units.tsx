    ...
    <div className="p-8 h-full overflow-y-auto">
      ...
      <div className="glass rounded-3xl p-6 border border-border">
        <table className="w-full text-left text-text-main">
          ...
          <tbody>
            {units.map(unit => (
              <tr key={unit.id} className="border-b border-border hover:bg-surface-dark transition-colors">
                ...
                <td className="py-4 text-right">
                  <button onClick={() => { setFormData(unit); setShowModal(true); }} className="text-blue-500 hover:underline mr-4 font-medium">Edit</button>
                  <button onClick={() => handleDelete(unit.id)} className="text-red-500 hover:underline font-medium">Delete</button>
                </td>
              </tr>
            ))}
            {units.length === 0 && (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-3xl p-8 w-full max-w-md border border-border shadow-2xl">
            <h2 className="text-2xl font-bold text-text-main mb-6">{formData.id ? 'Edit Unit' : 'Tambah Unit'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2 mt-4">
                <input type="checkbox" checked={formData.allow_decimal} onChange={e => setFormData({...formData, allow_decimal: e.target.checked})} className="w-5 h-5 rounded border-border bg-background text-brand-500 focus:ring-brand-500 focus:ring-offset-background" id="decimal" />
                <label htmlFor="decimal" className="text-text-main text-sm font-medium">Allow Decimal (Izinkan Desimal)</label>
              </div>
