...
export function Departments() {
  ...
  const fetchDepartments = async () => {
    try {
      const res = await fetch(`${API_URL}/items/departments`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = formData.id ? `${API_URL}/items/departments/${formData.id}` : `${API_URL}/items/departments`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await fetch(`${API_URL}/items/departments/${id}`, {
        method: 'DELETE',
  ...
  return (
    <div className="p-8 h-full overflow-y-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-bold text-text-main mb-2">Departments</h1>
          <p className="text-text-muted">Kelola department/divisi untuk sistem POS</p>
        </div>
        <button 
        ...
        <table className="w-full text-left text-text-main">
          ...
          <tbody>
            {departments.map(dept => (
              <tr key={dept.id} className="border-b border-[var(--color-border)] hover:bg-surface-light transition-colors">
                <td className="py-4 font-medium">{dept.name}</td>
                <td className="py-4 text-text-muted">{dept.description || '-'}</td>
                <td className="py-4 text-right">
                  <button onClick={() => { setFormData(dept); setShowModal(true); }} className="text-blue-400 hover:text-blue-300 mr-4 font-medium transition-colors">Edit</button>
                  <button onClick={() => handleDelete(dept.id)} className="text-red-400 hover:text-red-300 font-medium transition-colors">Delete</button>
                </td>
              </tr>
            ))}
            {departments.length === 0 && (
              <tr><td colSpan={3} className="py-8 text-center text-text-muted">Belum ada department.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300">
          <div className="glass rounded-3xl p-8 w-full max-w-md border border-[var(--color-border)] shadow-2xl transform transition-transform duration-300 scale-100">
            <h2 className="text-2xl font-bold text-text-main mb-6">{formData.id ? 'Edit Department' : 'Tambah Department'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-text-muted mb-2">Nama Department</label>
                <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="input-field w-full" placeholder="e.g. Kitchen, Bar, Cashier" />
              </div>
              <div>
