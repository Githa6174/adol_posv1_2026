  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-bold text-white mb-2">Variations</h1>
              ...
              <tr key={v.id} className="border-b border-white/5 hover:bg-white/5">
                ...
                <td className="py-4 text-text-muted">
                  <div className="flex gap-2 flex-wrap">
                    {v.options?.map((opt: any) => (
                      <span key={opt.id} className="bg-white/10 px-2 py-1 rounded text-xs">{opt.name}</span>
                    ))}
                  </div>
                </td>
                <td className="py-4 text-right">
                  <button onClick={() => { 
                    ...
                    setShowModal(true); 
                  }} className="text-blue-400 hover:text-blue-300 mr-4">Edit</button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass rounded-3xl p-8 w-full max-w-md border border-white/10 shadow-2xl">
            <h2 className="text-2xl font-bold text-white mb-6">{formData.id ? 'Edit Variasi' : 'Tambah Variasi'}</h2>
